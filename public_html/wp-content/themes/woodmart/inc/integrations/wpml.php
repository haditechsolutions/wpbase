...
if ( ! function_exists( 'woodmart_replace_gifts_ids_to_translated_id' ) ) {
	/**
	 * Get origin free gifts ids and replace to translated product id.
	 *
	 * @param int|string $free_gift_id Free product id.
	 *
	 * @return string
	 */
	function woodmart_replace_gifts_ids_to_translated_id( $free_gift_id ) {
		if ( ! defined( 'WCML_VERSION' ) ) {
			return $free_gift_id;
		}

		$current_lang = apply_filters( 'wpml_current_language', null );
		$free_gift_id = apply_filters( 'wpml_object_id', $free_gift_id, 'product', true, $current_lang );

		return strval( $free_gift_id );
	}

	add_filter( 'woodmart_free_gift_id', 'woodmart_replace_gifts_ids_to_translated_id' );
}
