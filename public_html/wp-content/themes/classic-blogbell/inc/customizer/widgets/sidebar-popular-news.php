...
function classic_blogbell_sidebar_popular_news() {
	register_widget( 'Classic_BlogBell_Sidebar_Popular_News' );
}
add_action( 'widgets_init', 'classic_blogbell_sidebar_popular_news' );

class Classic_BlogBell_Sidebar_Popular_News extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_sidebar_popular = array(
		  'classname'   => 'sidebar-popular-news',
		  'description' => esc_html__( 'Add Widget to Display Trending Posts.', 'classic-blogbell' )
		);
		parent::__construct( 'Classic_BlogBell_Sidebar_Popular_News',esc_html__( 'ST:Trending Posts', 'classic-blogbell' ), $widget_sidebar_popular, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'Trending Posts', 'classic-blogbell' ),		
			  'category'       	=> '', 
			  'number'          => 5, 
			  'show_category'	=> true,	
			) 
		);
		$title     			= isset( $instance['title'] ) ? esc_html( $instance['title'] ) : esc_html__( 'Trending Posts', 'classic-blogbell' );
		...
		<p>
			...
			<?php
				wp_dropdown_categories(array(
					'show_option_none' => '',
					'class' 		  => 'widefat',
					'show_option_all'  => esc_html__('Trending Posts','classic-blogbell'),
